...
그리고 DOM에서 Clock 을 삭제했을 때 타이머를 해제 하려하느데, React에서 이를 **unmounting** 이라고 부른다. 많은 컴포넌트를 가진 어플리케이션에서, 컴포넌트가 제거될 때 리소스를 풀어주는 건 아주 중요한 일이다.
